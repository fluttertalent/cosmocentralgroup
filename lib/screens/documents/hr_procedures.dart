...
class HrProcedures extends StatefulWidget {
  final category ;
  final location ;
  const HrProcedures({Key? key,this.category,this.location}) : super(key: key);
  @override
  State<HrProcedures> createState() => _HrProcedures();
}

class _HrProcedures extends State<HrProcedures> {
  ...
  gethrdocs() async {
    ...
    if (response.statusCode == 200) {
      ...
      for(var item in body){
        ...
        if(tmp !=false && item['categories'][0]['name'] == 'HR Procedures') {
          tempTileList.add(
              DocListTile(
