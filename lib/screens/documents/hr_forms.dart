...
class HrForms extends StatefulWidget {
  final category ;
  final location ;
  const HrForms({Key? key,this.category,this.location}) : super(key: key);
  @override
  State<HrForms> createState() => _HrForms();
}

class _HrForms extends State<HrForms> {
  ...
  gethrdocs() async {
    ...
    if (response.statusCode == 200) {
      ...
      for(var item in body){
        ...
        if(tmp !=false && item['categories'][0]['name'] == 'HR Documents') {
          tempTileList.add(
              DocListTile(
