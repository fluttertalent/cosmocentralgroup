...
class EventForms extends StatefulWidget {
  final category ;
  final location ;
  const EventForms({Key? key,this.category,this.location}) : super(key: key);
  @override
  State<EventForms> createState() => _EventForms();
}

class _EventForms extends State<EventForms> {
  ...
  gethrdocs() async {
    ...
    if (response.statusCode == 200) {
      ...
      for(var item in body){
        ...
        if(tmp !=false && item['categories'][0]['name'] == 'Event Documents') {
          tempTileList.add(
              DocListTile(
